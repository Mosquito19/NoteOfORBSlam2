...
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    const DBoW2::FeatureVector &vFeatVecKF = pKF->mFeatVec;///对于bow的正索引。。
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first) //步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                MapPoint* pMP = vpMapPointsKF[realIdxKF]; // 取出KF中该特征对应的MapPoint
												//一个特征对应一个mappoint？？？？？
                if(!pMP)                                                               ///那么就是第1个特征对应第1个mappoint点
                    continue;
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    ...
                    const int dist =  DescriptorDistance(dKF,dF); // 求这两个特征的描述子的距离
        ...
        else if(KFit->first < Fit->first)//注意lower_bound的作用，找到大于等于这个数的第一个位置。。
        {///比如KF中node值是 2 3 5 7 12 ，F中的node值是 6 7 8 19;那么直接找到 KF中大于等于6的位置，前面 2 3 5node都不用比了。
...
{
    int nmatches=0;
    vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);//前面一帧的特征点数

    vector<int> rotHist[HISTO_LENGTH];//vector<int> a[30] ;一个数组，每个元素为vector<int>
    for(int i=0;i<HISTO_LENGTH;i++)//30次
        rotHist[i].reserve(500);
    const float factor = 1.0f/HISTO_LENGTH;//   1/30

    vector<int> vMatchedDistance(F2.mvKeysUn.size(),INT_MAX);
    vector<int> vnMatches21(F2.mvKeysUn.size(),-1);//当前的所有特征，对应的特征序号

    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)//
    {
        cv::KeyPoint kp1 = F1.mvKeysUn[i1];//对于前一帧的图片的所有特征点，都遍历。。
        int level1 = kp1.octave;//F1的这个特征所在的 层数，一共8层
        if(level1>0)
            continue;//对于F2每个特征点,都遍历，得到它在网格中的特征数
            //F1第5个特征点对应的层数，在F2中去这个对应特征点对应的x y位置周围 去找 属于第minlevel这一层的特征点
            ///vbPrevMatched[i1].x,vbPrevMatched[i1].y就是F1中的kp点，找这个点的附近特征点////感觉这样有些局限。。只适合平移旋转小幅度的
        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y, windowSize,level1,level1);//windowsize=100
	//																		////	相应地，在F2中对应的层里面搜素。。
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            int dist = DescriptorDistance(d1,d2);////遍历计算d1与所有d2的距离

            if(vMatchedDistance[i2]<=dist)//dist比最大还大
                continue;

            if(dist<bestDist)
            {
                bestDist2=bestDist;//bestDist2 次好的那个
                bestDist=dist;
                bestIdx2=i2;
            }
            else if(dist<bestDist2)
        ...
        //// 类似于SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)函数步骤4
     if(bestDist<=TH_LOW)//匹配距离小于阈值50
        {
	   // 最佳匹配比次佳匹配明显要好，那么最佳匹配才真正靠谱
            if(bestDist<(float)bestDist2*mfNNratio)//Nearest Neighbor？？0.9
            {
                if(vnMatches21[bestIdx2]>=0)//防止这种情况：图2中的某个特征点 idx2，是图1的多个特征点的匹配。。
                {
                    vnMatches12[vnMatches21[bestIdx2]]=-1;///
                    nmatches--;
                }
                vnMatches12[i1]=bestIdx2;//F2中的最佳索引赋值给Matches
                vnMatches21[bestIdx2]=i1;//相应地，21里面，把1的索引放进去
                vMatchedDistance[bestIdx2]=bestDist;//最好的距离 放进去
                nmatches++;
...
// 所有的特征点的角度变化应该是一致的，通过直方图统计得到最准确的角度变化值
		    ...
		    //该特征点角度变化值
                    if(rot<0.0)
                        rot+=360.0f;
                    int bin = round(rot*factor);//将rot分配到bin组 factor=1/30 .( 0-360)/30=0-12
                    if(bin==HISTO_LENGTH)
                        bin=0;
                    assert(bin>=0 && bin<HISTO_LENGTH);
                    rotHist[bin].push_back(i1);//i1索引 放入这个rotHist[3]中,
                }
            }
        }
    ...
    if(mbCheckOrientation)
    {
        ...
        {   // 如果特征点的旋转角度变化量属于这三个组，则保留
            ...
            for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
            {
                ...
                if(vnMatches12[idx1]>=0)///肯定大于0啊。。。。
                {
                    vnMatches12[idx1]=-1;///??为啥 ？？哎。。。好吧 注意continue.....不符合 ind1 ind2 indi3的 才会被剔除。。。
                    nmatches--;
                }
            }
        }
    ...
    for(size_t i1=0, iend1=vnMatches12.size(); i1<iend1; i1++)
        if(vnMatches12[i1]>=0)
            vbPrevMatched[i1]=F2.mvKeysUn[vnMatches12[i1]].pt;///F2里面匹配上的pt点赋值给prevMatched
                                       ...
                                       vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo)
{
    ...
    cv::Mat C2 = R2w*Cw+t2w; // tc2c1 KF1的相机中心在KF2坐标系的表示,是空间坐标///注意了~~~~~~应该也可以用P*X表示
    const float invz = 1.0f/C2.at<float>(2);
    // 步骤0：得到KF1的相机光心在KF2中的坐标（极点坐标）
    const float ex =pKF2->fx*C2.at<float>(0)*invz+pKF2->cx;//空间坐标再乘以相机参数到图象坐标
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        const float maxDistance = pMP->GetMaxDistanceInvariance();
        const float minDistance = pMP->GetMinDistanceInvariance();
        const float dist3D = cv::norm(p3Dc2);///空间点，这个向量的长度
