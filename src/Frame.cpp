...
bool Frame::mbInitialComputations=true;////这一堆static的变量,在类外初始化!!!
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    if(mbInitialComputations)
    {
        ComputeImageBounds(imGray);////计算边界，mnMaxX等4个值。

        mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);
	//一般情况，64/640=0.1;一个格子宽0.1的意思。。
        mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/static_cast<float>(mnMaxY-mnMinY);
...
void Frame::AssignFeaturesToGrid()//分配特征点到网格里面去
{
	...
	//这样做有个好处，325~334 的10个数，都乘以0.1,变成33,后面都push进去。。所以，mGrid[i][j]存的是向量
    }
}
...
{
    pMP->mbTrackInView = false;///////////最后这些为false的不投影
    ...
    const float viewCos = PO.dot(Pn)/dist;////dot 点乘！！两个向量点乘
    ...
    // 标记该点将来要被投影
    pMP->mbTrackInView = true;//////标记该点将来要被投影，。。。。
    pMP->mTrackProjX = u;
...
/**
 * @brief 找到在 以x,y为中心,边长为2r的方形内且在[minLevel, maxLevel]的特征点---指定层了
 * @param x        图像坐标u
 * @param y        图像坐标v
 * @param r        边长
 * @param minLevel 最小尺度
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel) const
{
    ...
    const int nMinCellX = max(0,(int)floor((x-mnMinX-r)*mfGridElementWidthInv));///小的x坐标
    if(nMinCellX>=FRAME_GRID_COLS)//////边界判断
        return vIndices;
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)///0~64左右
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            const vector<size_t> vCell = mGrid[ix][iy];//这个二维数组 网格里的 放的特征的序号 组成的向量
            if(vCell.empty())
                continue;

            for(size_t j=0, jend=vCell.size(); j<jend; j++)//网格里的 这个向量里的特征序号 遍历
            {
                const cv::KeyPoint &kpUn = mvKeysUn[vCell[j]];
                if(bCheckLevels)
                {
                    if(kpUn.octave<minLevel)///如果这个特征不是要求的minlevel层，就放弃
                        continue;
                    if(maxLevel>=0)
                        if(kpUn.octave>maxLevel)//如果大于maxlevel层，也放弃
                            continue;
                }

                const float distx = kpUn.pt.x-x;///关键点的x减去 给定的x的距离值
                const float disty = kpUn.pt.y-y;

                if(fabs(distx)<r && fabs(disty)<r)////如果在半径r之内，就push进去。。。
...
{//这样做有个好处，325~334 的10个数，都乘以0.1,变成33,
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)//就是无畸变图象的边界。。畸变处理后，肯定边界会变化的
{
    if(mDistCoef.at<float>(0)!=0.0)
    {
        ...
        // Undistort corners
        mat=mat.reshape(2);////改变通道，不改变行数，成为2通道的，4行2列1通道，变为4行1列2通道
        cv::undistortPoints(mat,mat,mK,mDistCoef,cv::Mat(),mK);
        mat=mat.reshape(1);//变回1通道
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const float d = imDepth.at<float>(v,u);///说明 深度图已经和彩色图，配准了？？？？否则怎么可以直接赋值。。

        if(d>0)
        {
            mvDepth[i] = d;
            mvuRight[i] = kpU.pt.x-mbf/d;//////左右像素差值 Alpha*f*b/z
        }
    }
}
