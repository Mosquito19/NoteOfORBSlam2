                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    {
        if(vMatches12[i]>=0)//// vMatches12[i] 如果没匹配 值为-1;
	{
	  //注意 mvMatches12 类型是 vector<Match> ；Match类型是<int,int>
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(size_t j=0; j<8; j++)
        {
            ...
            // randi对应的索引已经被选过了，从容器中删除---防止重复！！！
            // randi对应的索引用最后一个元素替换，并删掉最后一个元素-------真巧妙。。
    ...
    // 步骤4：计算得分比例，选取某个模型
    float RH = SH/(SH+SF);///Sh 600多，sf1000多
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 利用重投影误差为当次RANSAC的结果评分------------循环200次,找到 currentScore最大的那个值 ,,这个值越大,就代表重映射误差 小于th阈值的多,匹配好
...
{
//     // Number of putative matches//假设的匹配 0个。
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(int j=0; j<8; j++)
        {
            ...
            vPn1i[j] = vPn1[mvMatches12[idx].first];///mvMatches12 值的形式：（8,29)，（30,394)。。。
            vPn2i[j] = vPn2[mvMatches12[idx].second];//first是vPn1[8]对应一个点，。。
        }
...
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2)
{
    ...
    cv::Mat A(2*N,9,CV_32F); // 2N*9 16*9;
    ...
    return vt.row(8).reshape(0, 3); // v的最后一列 reshape 成了3*3的矩阵 --->H得到
}
...
cv::Mat Initializer::ComputeF21(const vector<cv::Point2f> &vP1,const vector<cv::Point2f> &vP2)
{
    ...
    cv::Mat Fpre = vt.row(8).reshape(0, 3); // v的最后一行,1*9reshape成了3*3的矩阵
...
{   
    ...
    //信息矩阵，方差平方的倒数  msigma 初始化值为1 mpInitialzer中初始化的
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // x2in1 = H12*x2 /////////                         H12 是图2到图1的变换矩阵 
        // 将图像2中的特征点单应到图像1中
        // |u1|   |h11inv h12inv h13inv||u2|
        // |v1| = |h21inv h22inv h23inv||v2|
        // |1 |   |h31inv h32inv h33inv||1 |
        const float w2in1inv = 1.0/(h31inv*u2+h32inv*v2+h33inv);/////////相当于 x y z中的z .后面 x/z
        ...
        if(chiSquare2>th)////这个距离太大了,,,不进入累加啊.......................所以 距离不大的才累加进去了,,最后看累加结果大的,算是最好的一个H矩阵
            bIn = false;
        else
            score += th - chiSquare2;///////////score累加

        if(bIn)
            vbMatchesInliers[i]=true;///第i对匹配的点 匹配上了
        else
 ...
 * @param P2  投影矩阵P２
 * @param x3D 三维点
 * @see       Multiple View Geometry in Computer Vision - 11.2 Linear triangulation methods p312
 */
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
    ...
    // 将x坐标和y坐标分别进行尺度缩放，使得x坐标和y坐标的一阶绝对矩分别为1->就是说到原点的距离为根号2。
    for(int i=0; i<N; i++)
    {
