    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    if(sensor==System::RGBD)
    {
        ...
        if(fabs(mDepthMapFactor)<1e-5)
            ...
            mDepthMapFactor = 1.0f/mDepthMapFactor;////  1/5000;在tum数据集中。。。
    }
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)///mDepthMapFactor-1.0f)>1e-5？？
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        // mvIniMatches存储mInitialFrame,mCurrentFrame之间匹配的特征点//  mvIniMatches std::vector<int> 
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)///mvIniMatches[I]里面的值 -1 -1 383 385.。。。表示Mathcde索引值。
    {
        if(mvIniMatches[i]<0)
            continue;

        //Create MapPoint.
        cv::Mat worldPos(mvIniP3D[i]);///对应的 mvIniP3D[I]里面的值，就是3D点 （0,0,0) （0,0,0) （-1.5,0.7,25)。。。
        ...
        pKFini->AddMapPoint(pMP,i);///pkfini中是 AddMapPoint(pmp,4);而pkfcur中是AddMapPoint(pmp,384);
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]); ///关键帧里面添加地图点
    ...
    mpLocalMapper->InsertKeyFrame(pKFini);//////无缝跳转到了localMapping线程中~~~~~~~~~
...
bool Tracking::TrackLocalMap()
{
    ...
    SearchLocalPoints(); //调用 isInFrustum,调用 SearchByProjection（）;这个里面又要先判断mbTrackInView标志。。
    //所以 最终目的是为了获得更好 更多的 MapPoint
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // 步骤2：判断是否距离上一次插入关键帧的时间太短  mMaxFrames=fps。。一般情况为30帧
...
void Tracking::UpdateLocalMap()
{
    ...
    // 这行程序放在UpdateLocalPoints函数后面是不是好一些///设置参考的地图点
    mpMap->SetReferenceMapPoints(mvpLocalMapPoints);// mvpReferenceMapPoints = mvpLocalMapPoints;
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<MapPoint*>::const_iterator itMP=vpMPs.begin(), itEndMP=vpMPs.end(); itMP!=itEndMP; itMP++)
        {
            ...
            if(!pMP->isBad())
            {
                ...
                pMP->mnTrackReferenceForFrame=mCurrentFrame.mnId;///对每个push进去了的点做个标记，，，
            }
        }
    }
}
...
void Tracking::UpdateLocalKeyFrames()
{
        ...
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                ...
                for(map<KeyFrame*,size_t>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
                    keyframeCounter[it->first]++;///?什么用法？？--改变对应的keyframe* 的int 值，0,1,2,3~~~,
		    // (0xc489530, 104) (0xc4bcae0,101) (0xc4cc6a0,99) (0xca5f780,108) 数字的值表示，这个帧能看到的最多的点的个数
            }
            else
            {
    ...
    for(map<KeyFrame*,int>::const_iterator it=keyframeCounter.begin(), itEnd=keyframeCounter.end(); it!=itEnd; it++)
    {
        ...
        pKF->mnTrackReferenceForFrame = mCurrentFrame.mnId;//把每一个关键帧的mnTrackReferenceForFrame的值都变成当前的值
    }
...
bool Tracking::Relocalization()
{
    ...
    {
        mnLastRelocFrameId = mCurrentFrame.mnId;///////mnLastRelocFrameId更改值
        return true;
    }
