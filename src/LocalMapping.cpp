...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 将当前帧加入到闭环检测队列中---->无缝对接loopclosing线程~~~~~~~~~~~~~~~~~~
 ...
 * @param pKF KeyFrame
 */
void LocalMapping::InsertKeyFrame(KeyFrame *pKF)///在tracking.cpp的createNewKeyFrame函数中就有引用
{
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            cv::Mat ray2 = Rwc2*xn2;///////？？？？？？？一个世界点，乘以旋转矩阵？？？
...
cv::Mat LocalMapping::ComputeF12(KeyFrame *&pKF1, KeyFrame *&pKF2)
{
    ...
    cv::Mat R12 = R1w*R2w.t();///1对2的旋转=1对世界的旋转*世界对2的旋转。
    cv::Mat t12 = -R1w*R2w.t()*t2w+t1w;///t1w-R12*t2w;;;把t1固定，t2先旋转，完了再互相减，
    //就是相对距离了

    cv::Mat t12x = SkewSymmetricMatrix(t12);///反对称矩阵！！
